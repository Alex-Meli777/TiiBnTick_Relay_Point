...
export default function RelayDashboardPage() {
  ...
  // Pricing policy
  const [activeSection, setActiveSection] = useState<"dashboard" | "pricing">(
    "dashboard",
  );
  const [pricing, setPricing] = useState<RelayPointPricingPolicy | null>(null);
  // Load pricing when a point is selected
  useEffect(() => {
    if (selectedPointId) {
  ...
  const selectedPoint =
    owner?.relayPoints.find((rp) => rp.id === selectedPointId) ?? null;
          ...
          >
            <div className="rounded-3xl border border-orange-100 bg-white p-6 shadow-sm">
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                <CustomInput
                  label="Frais de garde / jour (FCFA)"
                  value={pricing?.pricePerDay}
                />
                <CustomInput
                  label="Délai gratuit (Jours)"
                  type="number"
    ...
    <div className="space-y-6 p-4 sm:p-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              {owner.relayPoints.length} point(s) relais associés
            </p>
          </div>
          <Link
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-4">
          ...
          <p className="text-2xl font-bold">
            {
              owner.relayPoints.filter(
                (rp) => rp.status === RelayPointStatusBackend.APPROVED,
              ).length
            }
          </p>
        </div>
        ...
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Cliquez sur un point relais pour consulter son tableau de bord.
            </p>
          </div>
        </div>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="rounded-full bg-orange-100 px-3 py-1 text-xs font-semibold text-orange-700">
                    {point.status === RelayPointStatusBackend.APPROVED
                      ? "Actif"
                      : point.status === RelayPointStatusBackend.SUSPENDED
                        ? "Suspendu"
                        : "En attente"}
                  </span>
                </div>
