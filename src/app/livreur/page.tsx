...
export function LivreurDashboard() {
  ...
  const fetchAvailableDeliveries = useCallback(async () => {
    ...
    try {
      ...
      setAvailableDeliveries(data.filter((a: any) => a.status === 'PUBLISHED'))
    } catch (error: any) {
  ...
  const fetchMyDeliveries = useCallback(async () => {
    ...
    try {
      ...
      const apiIds = data.map((a: any) => a.id)
      setSubscribedIds(prev => {
        const merged = new Set(prev)
        apiIds.forEach((id: string) => merged.add(id))
  ...
  const handleAcceptDelivery = async (deliveryId: string) => {
    if (!user?.id) return;
    try {
      await requestDelivery(deliveryId, user.id);
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          {activeTab === 'annonces' && (
            <>
              ...
              <Dialog open={detailsOpen} onOpenChange={(o: boolean) => { setDetailsOpen(o); if (!o) { setSelectedDelivery(null); } }}>
