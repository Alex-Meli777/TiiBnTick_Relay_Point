...
import { Package, UserPlusIcon } from 'lucide-react';
...
// 1. Update ShippingSteps order
const ShippingSteps = ({ currentStep = 1 }: { currentStep?: number }) => {
  const steps = [
    { number: 1, title: "Colis", icon: <Package className="w-5 h-5" />, color: "from-orange-400 to-orange-500" },
    { number: 2, title: "Expéditeur", icon: <UserOutlineIcon className="w-5 h-5" />, color: "from-orange-400 to-orange-600" },
    { number: 3, title: "Destinataire", icon: <UserOutlineIcon className="w-5 h-5" />, color: "from-orange-500 to-orange-600" },
...
export default function ShippingPage() {
  ...
  const [trackingNumber, setTrackingNumber] = useState("TBT-RP-000000");

  // Generate tracking number only on client side to avoid hydration mismatch
  useEffect(() => {
    if (formData.currentStep === 7 && trackingNumber === "TBT-RP-000000") {
      setTrackingNumber(`TBT-RP-${Date.now().toString().slice(-6)}`);
    }
    ...
    const checkUserAndLoadData = async () => {
      ...
      try {
        ...
        if (savedData) {
          ...
          try {
            ...
            console.warn(
              "Impossible de restaurer automatiquement le formulaire:",
              e,
            );
      ...
      if (session?.user) {
        const { data: profile } = await (supabase
          ...
          .single() as any);
        if (profile) {
          const connectedUser: LoggedInUser = {
            id: profile.id,
            full_name: profile.full_name,
            phone: profile.phone,
            email: profile.email,
            address: profile.address,
            lieu_dit: profile.lieu_dit,
          };
          ...
          if (!restoredData) {
            setFormData((prev) => ({
              ...
              senderData: {
                ...
                ...prev.senderData,
                senderFirstName: connectedUser.full_name
                  ? connectedUser.full_name.split(" ")[0]
                  : "",
                senderLastName: connectedUser.full_name
                  ? connectedUser.full_name.split(" ").slice(1).join(" ")
                  : "",
  ...
  // 2. Update renderCurrentStep inside ShippingPage
      ...
      case 3:
        return (
          <RecipientInfoStep
            ...
            excludeRpName={formData.senderData.senderAddress} // ADD THIS LINE
          />
        );
      case 4:
        return (
          <RouteSelectionStep
            // Route inputs dynamically handle addresses OR relay point names
            initialDepartureAddress={
              formData.senderData.senderAddress ||
              formData.senderData.senderCity
            }
            initialArrivalAddress={
              formData.recipientData.recipientAddress ||
              formData.recipientData.recipientCity
            }
            initialDepartureCoords={
              formData.senderData.latitude
                ? {
                    lat: formData.senderData.latitude,
                    lon: formData.senderData.longitude!,
                  }
                : undefined
            }
            initialArrivalCoords={
              formData.recipientData.recipientLatitude
                ? {
                    lat: formData.recipientData.recipientLatitude,
                    lon: formData.recipientData.recipientLongitude!,
                  }
                : undefined
            }
            ...
            onContinue={(routeData, travelPrice) =>
              setFormData((prev) => ({
                ...prev,
                routeData,
                pricing: { ...prev.pricing, travelPrice },
                currentStep: 5,
              }))
            }
              ...
              {!user && (
                ...
                >
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
                    Créez votre compte pour suivre vos colis, voir votre
                    historique et bénéficier d'offres exclusives. Vos
                    informations sont déjà prêtes !
                  </p>
                  <motion.button
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-orange-50 dark:from-gray-900 dark:to-gray-800">
      <main className="container mx-auto px-4 py-4">
        ...
        >
          ...
          <p className="mt-1 text-sm text-slate-600 dark:text-gray-300 max-w-xl mx-auto">
