...
export default function RecipientInfoStep({
  ...
  const mapCountryToKey = (countryName: string): string => {
    ...
    if (name.includes("cameroun") || name.includes("cameroon"))
      return "cameroun";
  ...
  const validateForm = () => {
    ...
    // ONLY validate manual addresses if a Relay Point is NOT selected
    if (!isRelayPoint) {
  ...
  const availableRegions = formData.recipientCountry
    ? countries[formData.recipientCountry as CountryKey]?.regions || {}
    : {};
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <RelayPointModal
                ...
                onSelect={(rp) => {
                  setSelectedRp(rp);
                  // Update form data to point to the relay point
                  setFormData((prev) => ({
                    ...prev,
                    senderAddress: rp.name, // We store the name to display on the map later
                    senderCity: "Yaoundé", // Assuming from mock
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      >
                        ...
                        {formData.recipientCity &&
                          !availableCities.includes(formData.recipientCity) && (
                            <option value={formData.recipientCity}>
                              {formData.recipientCity}
                            </option>
                          )}
                      </SelectField>
